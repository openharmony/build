...
def parse_args(args):
    ...
    parser.add_argument('--use-hvigor-obfuscation', help='use hvigor obfuscation', action='store_true')
...
def hvigor_build(cwd: str, options):
    ...
    if options.use_hvigor_obfuscation:
        cmd.extend(['-p', 'hvigor_obfuscation=true'])
    else:
        cmd.extend(['-p', 'hvigor_obfuscation=false'])
