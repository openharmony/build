...
def hvigor_write_log(cmd, cwd, env, hash_value):
    ...
    for line in stderr.splitlines():
        print(f"[2/2] [{hash_value}] Hvigor error: {line}")
    ...
    if proc.returncode or "ERROR: BUILD FAILED" in stderr or "ERROR: BUILD FAILED" in stdout:
        error_msg = "ReturnCode:{}. Hvigor build failed: {}".format(proc.returncode, stderr)
        print(f"[0/0] [{hash_value}] {error_msg}")
...
def set_sdk_path(cwd: str, model_version: str, options, env, node_home: str):
