...
def file_base_name(path: str):
    return os.path.splitext(os.path.basename(path))[0]


def pkg_to_crate_name(s: str):
...
def get_designated_pkg_info(lines: list, designated: str):
...
def short_out_name(pkg: str, s: str):
...
class Crate(object):
    ...
    def __init__(self, runner, outfile_name: str):
    ...
    def parse_rustc(self, line_num: int, line: str):
    ...
    def add_ignore_options_flag(self, flag: str):
    ...
    def deal_extern(self, arg: str):
        deps = re.sub('=/[^ ]*/deps/', ' = ', arg)
        self.deps.append(deps)
        self.core_deps.append(re.sub(' = .*', '', deps))

    def set_root_pkg_name(self, arg: str):
    ...
    def deal_static_and_dylib(self, arg: str):
    ...
    def decide_one_module_type(self, crate_type: str):
    ...
    def merge_crate(self, other, outfile_name: str):
    ...
    def dump_skip_crate(self, kind: str):
        if self.debug:
    ...
    def dump_one_gn_module(self, crate_type: str):
    ...
    def dump_gn_first_properties(self, crate_type: str):
        if crate_type != 'bin':
    ...
    def dump_gn_property_list(self, name: str, fmt, values):
        if not values:
            return
        if len(values) > 1:
            self.write('    ' + name + ' = [')
    ...
    def get_rust_lib(self, lib_name: str):
        rust_lib = ''
        if lib_name:
...
class Runner(object):
    ...
    def deal_cargo_cmd(self, cargo_out: str):
    ...
    def run_cargo_cmd(self, cargo_cmd: str, cargo_out: str):
        if self.dry_run:
    ...
    def parse(self, infile: str, outfile_name: str):
    ...
    def add_empty_test(self, name: str):
        if name == 'unittests':
            self.empty_unittests = True
        else:
            self.empty_tests.add(name)

    def add_deps_libname_map(self, line: str):
        line_list = line.split()
        if len(line_list) > 1:
            self.deps_libname_map[pkg_to_crate_name(line_list[0])] = line_list[0]

    def parse_cargo_out(self, infile: str, outfile_name: str):
    ...
    def assert_empty_rustc_line(self, line: str):
    ...
    def init_gn_file(self, name: str):
    ...
    def add_error_infos(self, in_tests: str, line: str):
        if in_tests:
            self.test_error_infos += '// '.join(line)
        else:
            self.error_infos += line

    def deal_rustc_command(self, line_num: str, rustc_line: str, line: str, outfile_name: str):
