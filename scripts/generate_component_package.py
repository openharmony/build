...
def _get_inner_kits_info(out_path):
    ...
    json_path = os.path.join(
        out_path + "/build_configs/parts_info/inner_kits_info.json"
    )
...
def _get_static_lib_path(args, json_data):
    ...
    real_static_lib_path = os.path.join(
        args.get("out_path"), "obj", split_label, json_data.get("out_name")
    )
...
def _copy_dir(src_path, target_path):
    ...
    for file in filelist_src:
        ...
        if os.path.isdir(path):
            ...
            path1 = os.path.join(target_path, file)
            with os.fdopen(
                os.open(path1, os.O_WRONLY | os.O_CREAT, mode=0o640), "wb"
            ) as write_stream:
...
def _copy_bundlejson(args):
    bundlejson_out = os.path.join(
        args.get("out_path"), "component_package", args.get("part_path")
    )
    print("bundlejson_out : ", bundlejson_out)
    if not os.path.exists(bundlejson_out):
        os.makedirs(bundlejson_out)
    bundlejson = os.path.join(
        args.get("root_path"), args.get("part_path"), "bundle.json"
    )
    if os.path.isfile(bundlejson):
        ...
        with os.fdopen(
            os.open(
                os.path.join(bundlejson_out, "bundle.json"),
...
def _copy_readme(args):
    ...
    readme_zh = os.path.join(
        args.get("root_path"), args.get("part_path"), "README_zh.md"
    )
    if os.path.isfile(readme_zh):
        shutil.copy(readme_zh, readme_out)
    readme_en = os.path.join(
        args.get("root_path"), args.get("part_path"), "README_en.md"
    )
...
def _get_parts_path_info(out_path):
    jsondata = ""
    json_path = os.path.join(
        out_path + "/build_configs/parts_info/parts_path_info.json"
    )
    with open(json_path, "r") as f:
        try:
            jsondata = json.load(f)
