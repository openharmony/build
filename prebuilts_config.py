...
def npm_install(dir):
    result = subprocess.run(["npm", "install", "--prefix", dir], capture_output=True, text=True)
    if result.returncode == 0:
        print("{}目录下npm install完毕".format(dir))
    else:
...
def install_hpm(code_path, download_dir, symlink_dir, home_path):
    ...
    print("-------------------start installing hpm----------------")
    try:
        subprocess.run(['node', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        print("Node.js not detected. It's recommended to install Node.js first.")
        choice = input("Continue installation? (yes/no): ")
        if choice.lower() != 'yes':
