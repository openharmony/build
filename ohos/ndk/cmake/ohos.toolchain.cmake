...
if(_DEVICEINFO_LIB)
  set(_OHOS_SHARED_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS} ${_DEVICEINFO_LIB}")
  set(_OHOS_MODULE_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS} ${_DEVICEINFO_LIB}")
  set(_OHOS_EXE_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS} ${OHOS_EXE_LINKER_FLAGS} ${_DEVICEINFO_LIB}")
else()
  set(_OHOS_SHARED_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS}")
  set(_OHOS_MODULE_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS}")
  set(_OHOS_EXE_LINKER_BASE "${OHOS_COMMON_LINKER_FLAGS} ${OHOS_EXE_LINKER_FLAGS}")
endif()

# Strip OHOS default flags from cached CMAKE_*_LINKER_FLAGS using marker boundaries.
# On reconfigure, extract user flags from _OHOS_*_LINKER_FLAGS_META via fixed markers,
# which works regardless of base content changes (OHOS_STL / OHOS_ALLOW_UNDEFINED_SYMBOLS /
# OHOS_COMPATIBLE_SDK_VERSION). On first configure (no META), fall back to content matching.
# If CMAKE_*_LINKER_FLAGS was externally modified (e.g., via -D on command line),
# detect the change by comparing with _OHOS_*_LINKER_FLAGS_EXPECTED and extract new user flags.
# Using UNINITIALIZED cache type allows -DVAR=value overrides on reconfigure (type matches).
if(_OHOS_SHARED_LINKER_FLAGS_META)
  if(DEFINED _OHOS_SHARED_LINKER_FLAGS_EXPECTED AND NOT "${CMAKE_SHARED_LINKER_FLAGS}" STREQUAL "${_OHOS_SHARED_LINKER_FLAGS_EXPECTED}")
    if(_OHOS_SHARED_LINKER_BASE)
    ...
    string(REGEX REPLACE ".*_OHOS_BASE_MARKER_END_" "" _OHOS_USER_SHARED_LINKER_FLAGS "${_OHOS_SHARED_LINKER_FLAGS_META}")
  endif()
...
if(_OHOS_MODULE_LINKER_FLAGS_META)
  if(DEFINED _OHOS_MODULE_LINKER_FLAGS_EXPECTED AND NOT "${CMAKE_MODULE_LINKER_FLAGS}" STREQUAL "${_OHOS_MODULE_LINKER_FLAGS_EXPECTED}")
    if(_OHOS_MODULE_LINKER_BASE)
    ...
    string(REGEX REPLACE ".*_OHOS_BASE_MARKER_END_" "" _OHOS_USER_MODULE_LINKER_FLAGS "${_OHOS_MODULE_LINKER_FLAGS_META}")
  endif()
...
if(_OHOS_EXE_LINKER_FLAGS_META)
  if(DEFINED _OHOS_EXE_LINKER_FLAGS_EXPECTED AND NOT "${CMAKE_EXE_LINKER_FLAGS}" STREQUAL "${_OHOS_EXE_LINKER_FLAGS_EXPECTED}")
    if(_OHOS_EXE_LINKER_BASE)
    ...
    string(REGEX REPLACE ".*_OHOS_BASE_MARKER_END_" "" _OHOS_USER_EXE_LINKER_FLAGS "${_OHOS_EXE_LINKER_FLAGS_META}")
  endif()
...
if(_OHOS_USER_SHARED_LINKER_FLAGS)
  string(APPEND _SHARED_LINKER_FLAGS " ${_OHOS_USER_SHARED_LINKER_FLAGS}")
endif()
set(CMAKE_SHARED_LINKER_FLAGS "${_SHARED_LINKER_FLAGS}" CACHE UNINITIALIZED "Linker flags to be used to create shared libraries." FORCE)
set(_OHOS_SHARED_LINKER_FLAGS_EXPECTED "${_SHARED_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)

set(_MODULE_LINKER_FLAGS "${_OHOS_MODULE_LINKER_BASE}")
if(_OHOS_USER_MODULE_LINKER_FLAGS)
  string(APPEND _MODULE_LINKER_FLAGS " ${_OHOS_USER_MODULE_LINKER_FLAGS}")
endif()
set(CMAKE_MODULE_LINKER_FLAGS "${_MODULE_LINKER_FLAGS}" CACHE UNINITIALIZED "Linker flags to be used to create modules." FORCE)
set(_OHOS_MODULE_LINKER_FLAGS_EXPECTED "${_MODULE_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)

set(_EXE_LINKER_FLAGS "${_OHOS_EXE_LINKER_BASE}")
if(_OHOS_USER_EXE_LINKER_FLAGS)
  string(APPEND _EXE_LINKER_FLAGS " ${_OHOS_USER_EXE_LINKER_FLAGS}")
endif()
set(CMAKE_EXE_LINKER_FLAGS "${_EXE_LINKER_FLAGS}" CACHE UNINITIALIZED "Linker flags to be used to create executables." FORCE)
set(_OHOS_EXE_LINKER_FLAGS_EXPECTED "${_EXE_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)

# Persist marker metadata so next configure can extract user flags via fixed markers,
# even if base content changes due to OHOS_STL / OHOS_ALLOW_UNDEFINED_SYMBOLS /
# OHOS_COMPATIBLE_SDK_VERSION being modified.
set(_OHOS_SHARED_LINKER_FLAGS_META "_OHOS_BASE_MARKER_BEGIN_ ${_OHOS_SHARED_LINKER_BASE} _OHOS_BASE_MARKER_END_ ${_OHOS_USER_SHARED_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)
set(_OHOS_MODULE_LINKER_FLAGS_META "_OHOS_BASE_MARKER_BEGIN_ ${_OHOS_MODULE_LINKER_BASE} _OHOS_BASE_MARKER_END_ ${_OHOS_USER_MODULE_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)
set(_OHOS_EXE_LINKER_FLAGS_META "_OHOS_BASE_MARKER_BEGIN_ ${_OHOS_EXE_LINKER_BASE} _OHOS_BASE_MARKER_END_ ${_OHOS_USER_EXE_LINKER_FLAGS}" CACHE INTERNAL "" FORCE)
