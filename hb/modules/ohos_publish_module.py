...
class OHOSPublishModule(PublishModuleInterface):
    ...
    def __init__(self, args_dict: dict, args_resolver: ArgsResolverInterface, hpm: BuildFileGeneratorInterface):
        ...
        self._hpm = hpm
        OHOSPublishModule._instance = self

    @staticmethod
    def get_instance():
        if OHOSPublishModule._instance is not None:
            return OHOSPublishModule._instance
        else:
            raise OHOSException(
                'OHOSPublishModule has not been instantiated', '0000')
    ...
    def _publish(self):
        self._run_phase()
        self.hpm.execute_hpm_cmd(CMDTYPE.PUBLISH)
    ...
    def run(self):
        try:
            ...
            LogUtil.hb_info('hb publish failed')
