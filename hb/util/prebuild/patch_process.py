...
class Patch():
    ...
    def patch_make(self, cmd_args=None, reverse=False):
        ...
        if not os.path.isfile(patch_cfg):
            raise OHOSException(f'{patch_cfg} not found, '
                                'which "--patch" parameter needs', '0020')

        patch_cfg_dict = IoUtil.read_yaml_file(patch_cfg)
    ...
    def patch_apply(self, src_path, patch_list, reverse=False):
        ...
        if not os.path.exists(src_path):
            raise OHOSException(f'{src_path} not exist, stop applying patch', '0021')

        cmd = ''
        for patch_item in patch_list:
            patch_path = os.path.join(self.config.root_path, patch_item)
            if not os.path.isfile(patch_path):
                patch_path = os.path.join(self.config.root_path,
                                          src_path, patch_item)
                if not os.path.isfile(patch_path):
                    raise OHOSException(f'patch {patch_item} not exist for {src_path}, '
                                        'stop applying patch', '0022')
            if reverse:
