...
def gen_symbols(tmp_file, sort_lines, symbols_path):
    ...
    with os.fdopen(os.open(tmp_file, os.O_WDWR | os.O_CREAT | os.O_TRUNC, modes), 'w', encoding='utf-8') as output_file:
        for item in sort_lines:
            output_file.write('{}\n'.format(item))

    with os.fdopen(os.open(symbols_path, os.O_WDWR | os.O_CREAT | os.O_TRUNC, modes), 'w', encoding='utf-8') as output_file:
...
def remove_adlt_postfix(llvm_objcopy_path, keep_path, mini_debug_path):
    ...
    with os.fdopen(os.open(rename_rules_path, os.O_WDWR | os.O_CREAT | os.O_TRUNC, modes), 'w', encoding='utf-8') as output_file:
...
def create_mini_debug_info(binary_path, stripped_binary_path, root_path, clang_base_dir, adlt_llvm_tool):
    ...
    modes = stat.S_IWUSR | stat.S_IRUSR | stat.S_IWGRP | stat.S_IRGRP
    with os.fdopen(os.open(tmp_file1, os.O_WDWR | os.O_CREAT | os.O_TRUNC, modes), 'w', encoding='utf-8') as output_file:
    ...
    with os.fdopen(os.open(keep_path, os.O_WDWR | os.O_CREAT | os.O_TRUNC, modes), 'w', encoding='utf-8') as output_file:
