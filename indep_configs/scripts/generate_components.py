...
def _get_bundle_path(hpm_cache_path, dependences_json, part_name):
    bundle_path = (
        hpm_cache_path
        + dependences_json[part_name]["installPath"]
        + os.sep
        + "bundle.json"
    )
    return bundle_path
...
def _link_kernel_binarys(variants, hpm_cache_path, dependences_json):
    ...
    musl_include_link_path = os.path.join(
        "out", variants, "obj/binarys/third_party/musl/usr/include/arm-linux-ohos"
    )
    musl_lib_link_path = os.path.join(
        "out", variants, "obj/binarys/third_party/musl/usr/lib/arm-linux-ohos"
    )
    os.makedirs(musl_include_link_path, exist_ok=True)
    os.makedirs(musl_lib_link_path, exist_ok=True)
    _symlink_src2dest(
        musl_real_path + os.sep + "innerapis/includes", musl_include_link_path
    )
...
def _gen_components_info(components_json, bundle_json, part_name, src_build_name_list):
    ...
    for i in component:
        ...
        if part_name == "musl":
            innerapi_label = "{}:{}".format(
                os.path.join("//binarys", path), innerapi_name
            )
        elif part_name in src_build_name_list:
            innerapi_label = i["name"]
        else:
            innerapi_label = "{}:{}".format(
                os.path.join("//binarys", path, "innerapis", innerapi_name),
                innerapi_name,
            )
...
def _components_info_handler(
    part_name_list, source_code_path, hpm_cache_path, root_path, dependences_json
):
    ...
    components_json = _gen_components_info(
        components_json,
        utils.get_json(src_bundle_path),
        src_part_name,
        src_build_name_list,
    )
    components_json = _gen_components_info(
...
def _generate_platforms_list(output_path):
    ...
    gni_file_content = [
        f'target_platform_list = [ "{platforms_list_strings}" ]',
...
def main():
    ...
    output_part_path = os.path.join(
        project_path, "out", variants, "build_configs", "parts_info"
    )
    ...
    components_json = _components_info_handler(
        part_name_list, source_code_path, hpm_cache_path, root_path, dependences_json
    )
